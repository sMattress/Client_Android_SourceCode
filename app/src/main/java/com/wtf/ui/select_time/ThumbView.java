...
public class ThumbView extends ImageView {
    ...
    private float mCenterX;   //游标的中心位置
    ...
    public void setCenterX(float centerX) {
        float left1=(centerX-mWidth/2.f),right1=(centerX+mWidth/2.f);
        if(centerX<mLeftLimit) {
            left1=mLeftLimit-mWidth/2;
            right1=mLeftLimit+mWidth/2;
        }

        if(centerX>mRightLimit){
            left1=mRightLimit-mWidth/2;
            right1=mRightLimit+mWidth/2;
        }

        this.mCenterX = (left1+right1)/2;
        int left = (int)(left1+0.5), right = (int)(right1+0.5);
        if(left!=rect.left||right!=rect.right){
            ...
            if(listener!=null){
                listener.onThumbChange(100*((left+right)/2-mLeftLimit)/(mRightLimit-mLeftLimit+100));
            }
        }
        rangeSeekBar.invalidate();
    }
